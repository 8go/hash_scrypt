...
Uses Python package cryptography (pyca/cryptography) for implementation.
...
class cryptography.hazmat.primitives.kdf.scrypt.Scrypt(
                salt, length, n, r, p, backend=None)
...
Comparison:
In comparison with passlib (hash_scrypt_passlib.py),
pyca/cryptography (hash_scrypt_pycacrypto.py) has 3 advantages:
...
hash_scrypt_pycacrypto.py  'my passphrase' --salt-base64 'YmF0dGVyeSBob3JzZSBzdGFwbGU'
...
def hash(  # noqa
    ...
    if salt_length is not None:
        salt = os.urandom(salt_length)  # returns bytes
    elif salt_utf8 is not None:
        salt = salt_utf8.encode("utf-8")  # s-to-b
    ...
    salt_base64_bytes = base64.b64encode(salt)
    salt_base64_string = salt_base64_bytes.decode("utf-8")  # b-to-s
    # just double-checking
    if (salt_base64 is not None) and (salt_base64 != salt_base64_string):
        logging.warning(
            f"{fnln()}:: ❗ Warning. The salt you provided is a valid base64 "
            "encoding but it is strange because it has not been generated "
            "by a standard base64 encoding algorithm. "
            f"Your base64 string is {salt_base64}. But the "
            f"standard encoding of this string is {salt_base64_string}. "
            "Everything will work but be warned. If in doubt abort and use "
            f"{salt_base64_string} instead."
        )
    passphrase_bytes = passphrase.encode("utf-8")  # s-to-b
    # len(salt) mod 3 == 0 ... no pad, % 3 == 1 ... 2 pads, % 3 == 2 ... 1 pad
    no_of_pads = (3 - (len(salt) % 3)) % 3
    if no_of_pads != 0:
        # remove pads, remove last char(s)
        salt_base64_string = salt_base64_string[:-no_of_pads]
        if no_of_pads == 1:
            txt = "pad was"
        else:
            txt = "pads were"
        logging.debug(
            f"{fnln()}:: Salt length is {len(salt)} bytes. {no_of_pads} {txt} "
            "removed from end of base64 encoded salt."
        )
        logging.warning(
            f"{fnln()}:: ❗ other implementations MIGHT not remove the "
            "bases64 padding characters at the end of the base64 encoded "
            "salt string! It will not be compatible with those apps."
        )
    # derive, create a KDF instance
    # The next 2 lines are the only 2 lines of code that count in this
    # whole program. All other lines are just fluff.
    kdf = Scrypt(salt=salt, length=key_length, n=n, r=r, p=p)
    key = kdf.derive(passphrase_bytes)  # returns bytes

    key_base64_bytes = base64.b64encode(key)
    key_base64_string = key_base64_bytes.decode("utf-8")  # b-to-s
    for idx, value in {
        "kdf": kdf,  # kdf.scrypt.Scrypt
        "salt": salt,  # bytes
        "passphrase": passphrase,
        "passphrase_bytes": passphrase_bytes,
        "key": key,  # bytes
        "key_base64_bytes": key_base64_bytes,
        "key_base64_string": key_base64_string,
    }.items():
    ...
    # len(key) mod 3 == 0 ... no pad, % 3 == 1 ... 2 pads, % 3 == 2 ... 1 pad
    no_of_pads = (3 - (len(key) % 3)) % 3
    if no_of_pads != 0:
        # remove pads, remove last char(s)
        key_base64_string = key_base64_string[:-no_of_pads]
        if no_of_pads == 1:
            txt = "pad was"
        else:
            txt = "pads were"
        logging.debug(
            f"{fnln()}:: Key length is {len(key)} bytes. {no_of_pads} {txt} "
            "removed from end of base64 encoded key."
        )
        logging.warning(
            f"{fnln()}:: ❗ other implementations MIGHT not remove the "
            "bases64 padding characters at the end of the base64 encoded "
            "key string! It will not be compatible with those apps."
        )

    if verify:
        # passphrase verification can be done like this
        kdfverify = Scrypt(salt=salt, length=key_length, n=n, r=r, p=p)
        try:
            kdfverify.verify(passphrase_bytes, key)  # raises exception
        except Exception as e:
            logging.error(
                f"{fnln()}:: ❌ Severe ERROR: verification of hash failed: {e}"
            )
            sys.exit(4)
        else:
    ...
    # create PHC / MCF format
    # MCF: Modular Crypt Format:
    # https://passlib.readthedocs.io/en/stable/modular_crypt_format.html
    # PHC spec:
    # https://github.com/P-H-C/phc-string-format/blob/master/phc-sf-spec.md
    # see also https://passlib.readthedocs.io/en/stable/lib/
    #                        passlib.hash.scrypt.html?highlight=scrypt
    # $<id>[$<param>=<value>(,<param>=<value>)*][$<salt>[$<hash>]]
    # $scrypt$ln=16,r=8,p=1$aM15713r3...$nFNh2CVHVjNldFVKDHDlm...
    hash_phc = (
        f"$scrypt$ln={ln},r={r},p={p}${salt_base64_string}${key_base64_string}"
    )
    algo_str = f"Algorithm and arguments are $scrypt$ln={ln},r={r},p={p}$"
    salt_str = f"Salt (as Base64-encoded string in UTF-8) is:        {salt_base64_string}"  # noqa
    key_str = f"Derived Key (as Base64-encoded string in UTF-8) is: {key_base64_string}"  # noqa
    saltlen_str = f"Salt length is {len(salt)} bytes."
    keylen_str = f"Key length is {key_length} bytes."
    keyphc_str = (
        f"✅ Hash in PHC Format is {hash_phc}   [used pyca/cryptography]"
    )
    partialphc_str = f"$scrypt$ln={ln},r={r},p={p}${salt_base64_string}$"
    if terse:
    ...
    if metadata_textio is not None:
        for el in [
            algo_str,
            salt_str,
            # key_str,  # let's NOT write the derived key to metadata file
            saltlen_str,
            keylen_str,
            # keyphc_str,  # let's NOT write the derived key to metadata file
            partialphc_str,
        ]:
            print(el, file=metadata_textio, flush=True)
    return hash_phc
...
def run_test_case9(number: int, match_expected: bool):
    ...
    reference = "$scrypt$ln=14,r=8,p=1$U29kaXVtQ2hsb3JpZGU$cCO9yzr9c0hGHAbNgf046/2o+7qQT44+qbVD9lRdofLVQylVYT8Pz2LUlwUkKpr55h6F3A1lHkDfzwF7RVdYhw"  # noqa
    match = result == reference
    if match == match_expected:
        logging.info(f"{fnln()}:: ✅ test case {number} passed. 😀")
        # logging.info(f"{fnln()}:: result    = {result}")
...
def run_test_case10(number: int, match_expected: bool):
    ...
    reference = "$scrypt$ln=4,r=1,p=1$$d9ZXYjhleyA7GcpCwYoEl/FrSETjB0ro39/6P+3iFEL80Aad7QlI+DJqdToPyB8X6NPg+y4NNijPNeIMONGJBg"  # noqa
    match = result == reference
    if match == match_expected:
        logging.info(f"{fnln()}:: ✅ test case {number} passed. 😀")
        # logging.info(f"{fnln()}:: result    = {result}")
...
def run_test_case11(number: int, match_expected: bool):
    ...
    reference = "$scrypt$ln=10,r=8,p=16$TmFDbA$/bq+HJ00cgB4VucZDQHp/nxq18vII3gw53N2Y0s3MWIurzDZLiKjiG/xCSedmDDaxyevuUqD7m2DYMvfoswGQA"  # noqa
    match = result == reference
    if match == match_expected:
        logging.info(f"{fnln()}:: ✅ test case {number} passed. 😀")
        # logging.info(f"{fnln()}:: result    = {result}")
...
def run_test_case12(number: int, match_expected: bool):
    ...
    reference = "$scrypt$ln=20,r=8,p=1$U29kaXVtQ2hsb3JpZGU$IQHLm2pRGq6t274Jz3D4gexWjVdKL/1Nq+XumCCtqkeOVv2PS6XQn/ocbZJ8QPTDNzBASeipUvvL9Fxvp3pBpA"  # noqa
    match = result == reference
    if match == match_expected:
        logging.info(f"{fnln()}:: ✅ test case {number} passed. 😀")
        # logging.info(f"{fnln()}:: result    = {result}")
